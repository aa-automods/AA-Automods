...
function createShootingStar() {
    ...
    const duration = Math.random() * 0.3 + 1;
...
function showSlide(n) {
    slides.forEach((slide, index) => {
        ...
        videos.forEach(video => {
            if (index === n) { video.play().catch(e => console.log("Video autoplay prevented:", e)) } else {
                video.pause();
                video.currentTime = 0
            }
        })
    ...
    function updateImages() {
        ...
        mediaItems.forEach((item, index) => {
            if (item.tagName === 'VIDEO') {
                if (index === currentIndex) { item.play().catch(e => console.log("Video autoplay prevented:", e)) } else {
                    item.pause();
                    item.currentTime = 0
                }
            }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (target) {
                scroll.scrollTo(target, { offset: -100, duration: 1000 });
                const nav = document.getElementById("main-nav");
    ...
    function debounce(func, wait = 20, immediate = !0) {
        let timeout;
        return function() {
            const context = this,
                args = arguments;
...
if (document.getElementById('paypal-button-container')) {
    paypal.Buttons({
        ...
        createOrder: function(data, actions) { return actions.order.create({ purchase_units: [{ description: "Deposit Payment", amount: { value: '100.00' } }] }) },
        onApprove: function(data, actions) { return actions.order.capture().then(function(details) { window.location.href = "https://yourdomain.com/thank-you" }) },
